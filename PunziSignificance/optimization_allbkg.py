...
for i in range(len(bkgname)):
    ...
    countBkgLnorm = countBkgL*L*xsBkg[i]/eventsbkg
    countBkgM1norm = countBkgM1*L*xsBkg[i]/eventsbkg
    countBkgM2norm = countBkgM2*L*xsBkg[i]/eventsbkg
    ...
    countBkgLnorm += countBkgLnorm
    countBkgM1norm += countBkgM1norm
    countBkgM2norm += countBkgM2norm
...
errPS_L = arr.array('d', [0.001469139750202249, 0.00165143426948704, 0.0014279560004221282, 0.0013233435572312176, 0.0016599166856341506, 0.0016106192094511053, 0.00190458178698159, 0.00303542451181524])
errPS_M1 = arr.array('d', [0.0016976365541714015, 0.0020112935630617287, 0.0018465069739237408, 0.0023117664671910426, 0.004745208117637165, 0.004967032509083927, 0.0050421673807105605, 0.0047808097079015364])
errPS_M2 = arr.array('d', [0.0013853897177720678, 0.0015209652664772877, 0.0016116644808070063, 0.0016692346320885826, 0.0037608327715817247, 0.004135673251560708, 0.00425771283301073, 0.0045431285362842735])
...
c2.cd()
c2.Update()
c2.SaveAs("SignalEfficiency_new.pdf")
...
mg3 = TMultiGraph("mg3","")
mg3.SetTitle("Nbkg in Data After Deep Double B-Tagger; Fat Jet CSV; Number of Events")
