...
h_FJetMassBkgL_norm = TH1F("h_FJetMassBkgL_norm","Background FJetMass Loose",40,0.6,1.0)
h_FJetMassBkgM1_norm = TH1F("h_FJetMassBkgM1_norm","Background FJetMass Medium1",40,0.6,1.0)
h_FJetMassBkgM2_norm = TH1F("h_FJetMassBkgM2_norm","Background FJetMass Medium2",40,0.6,1.0)
...
for i in range(len(bkgname)):
    ...
    h_FJetMassBkgL_norm = h_FJetMassBkgL*(L*xsBkg[i]/eventsbkg)
    h_FJetMassBkgM1_norm = h_FJetMassBkgM1*(L*xsBkg[i]/eventsbkg)
    h_FJetMassBkgM2_norm = h_FJetMassBkgM2*(L*xsBkg[i]/eventsbkg)
    
    h_FJetMassBkgL_norm += h_FJetMassBkgL_norm
    h_FJetMassBkgM1_norm += h_FJetMassBkgM1_norm
    h_FJetMassBkgM2_norm += h_FJetMassBkgM2_norm
    
    h_FJetMassBkgL.Reset()
    h_FJetMassBkgM1.Reset()
    h_FJetMassBkgM2.Reset()

h_FJetMassBkgL_norm.SetLineColor(1)
h_FJetMassBkgL_norm.SetLineWidth(4)
h_FJetMassBkgL_norm.SetMarkerStyle(20)
h_FJetMassBkgL_norm.SetMarkerColor(1)
h_FJetMassBkgL_norm.GetXaxis().SetTitle("Fat Jet CSV")
h_FJetMassBkgL_norm.GetYaxis().SetTitle("Number of Events")
h_FJetMassBkgL_norm.SetTitle("Nbkg in Data After Deep Double B-Tagger")

h_FJetMassBkgM1_norm.SetLineColor(2)
h_FJetMassBkgM1_norm.SetLineWidth(4)
h_FJetMassBkgM1_norm.SetMarkerStyle(21)
h_FJetMassBkgM1_norm.SetMarkerColor(2)

h_FJetMassBkgM2_norm.SetLineColor(4)
h_FJetMassBkgM2_norm.SetLineWidth(4)
h_FJetMassBkgM2_norm.SetMarkerStyle(22)
h_FJetMassBkgM2_norm.SetMarkerColor(4)

h_FJetMassBkgL_norm.Draw("e")
h_FJetMassBkgM1_norm.Draw("esame")
h_FJetMassBkgM2_norm.Draw("esame")

leg.AddEntry(h_FJetMassBkgL_norm, "Loose (FJetCSV > 0.7)")
leg.AddEntry(h_FJetMassBkgM1_norm, "Medium1 (FJetCSV > 0.86)")
leg.AddEntry(h_FJetMassBkgM2_norm, "Medium2 (FJetCSV > 0.89)")
leg.Draw()
