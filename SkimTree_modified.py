...
def runbbdm(txtfile):
    ...
    for df in read_root(filename, 'tree/treeMaker', columns=jetvariables, chunksize=125000):
		...
		n = len(pass_jet_index_cleaned)
                for a in range(n):
			if n == 1:
				jetP4 = TLorentzVector(ak4px_[ithinjet],ak4py_[ithinjet],ak4pz_[ithinjet],ak4e_[ithinjet])
				jetInvMass = jetP4.M()
				#print ('n',n,'jet',a,'=',jetInvMass)
				st_THINjetCanMass.push_back(jetInvMass)
			else:
				#print ('jet=',n,'--------')
				...
				for b in range(n):
					c = a+1+b
					if c < n:
						jet2 = pass_jet_index_cleaned[c]
						jet2_P4 = TLorentzVector(ak4px_[c],ak4py_[c],ak4pz_[c],ak4e_[c])
						jetsInvMass = (jet1_P4+jet2_P4).M()
						st_THINjetCanMass.push_back(jetsInvMass)
						#print ('n',n,'jet',b,'=',jetsInvMass)
					else:
						break
